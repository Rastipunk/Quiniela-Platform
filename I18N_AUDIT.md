...
- **Status:** 🟩 FIXED in `93637bf` (2026-05-22) — implemented Option (B) but keyed by **FIFA 3-letter code** (`MEX`, `BRA`, `KOR`, ...) instead of internal team id (`t_A1`). FIFA codes are stable across tournaments (the same Mexico appears in WC2026 and a future Copa America with the same `MEX` key), so the catalog scales without rekeying. Added `messages/{es,en,pt}/teams.json` (48 nations seeded), registered the `teams` namespace in `i18n/request.ts`, and added `getTeamName(team, t)` to `poolHelpers.ts`. Updated PickComponents, ResultComponents, KnockoutMatchCard, and groupStandings/TeamListComponents to use the helper. UCL clubs fall through to the dataJson `team.name` (proper-noun names rarely change between locales). Backend already returns `code` on `PoolTeamRef` so no API change was needed.
