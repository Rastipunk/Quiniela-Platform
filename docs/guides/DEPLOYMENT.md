...
| Variable | Description | Default |
|----------|-------------|---------|
| `PREDICTION_STATUS_HOST_ALLOWLIST` | Gradual rollout for the per-match prediction-status feature (ADR-045). `""`/unset → off everywhere; `*` → on for all pools; comma-separated emails → on only for pools whose creator's email is listed. | `""` (off) |
