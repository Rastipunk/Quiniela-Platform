...
- **A release is the SINGLE SOURCE OF TRUTH (instance + pools).** On release, `setKnockoutPhaseReleased` writes the reviewed bracket into BOTH `instance.dataJson` (`propagateBracketToInstance`) AND every pool's `fixtureSnapshot` (`propagateBracketToPools`). The instance write matters because **pool creation copies `instance.dataJson`** (`routes/pools.ts`, `corporateService.ts`) — without it, a pool created *after* release inherits the raw auto-resolved bracket. Now any new pool is born with the admin-approved teams.
- **Auto-advance never overwrites a RELEASED phase.** A released bracket is admin-authoritative. `advanceToRoundOf32` / `advanceKnockoutPhase` skip the `fixtureSnapshot` write (and scraper-mapping persist) when the target phase is in `releasedKnockoutPhases` (gate on), via `isKnockoutPhaseReleased`; `validateCanAutoAdvance` early-exits for the same condition so the 10-min timer doesn't even fire. **Why this exists (2026-06-28 incident):** after the World Cup R32 release, 9 pools whose auto-advance timer fired *after* the release got their corrected bracket clobbered by the automatic resolution — the screenshot showed "Colombia vs Iran" (Iran did NOT qualify; Algeria did). Root cause: `resolvePlaceholders` **assigns the 8 best-thirds to R32 slots by RANK (`3rd_POOL_n → bestThirds[n-1]`)**, but the FIFA 48-team format assigns thirds to slots by a GROUP-COMBINATION table — so the *automatic* R32 best-thirds matchups are wrong and MUST be admin-reviewed (`determineQualifiers` picks the correct 8; only the slot allocation is wrong). The 9 pools + `instance.dataJson` were repaired by re-running `propagateBracketToPools` / `propagateBracketToInstance` (idempotent; matchIds unchanged so existing score predictions carry over). The deeper `resolvePlaceholders` best-thirds-allocation fix is tracked in TECH_DEBT — the Gestor review + release-as-source-of-truth + this guard make it a non-correctness issue (it only affects the auto DRAFT the admin reviews).
