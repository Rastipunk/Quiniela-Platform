...
#### Added
- **"Mantener sesión abierta en este dispositivo"** — un único checkbox **arriba del todo** del panel de login (siempre visible), **desmarcado por defecto** (opt-in). Gobierna **ambos métodos**: inicio con Google e inicio con correo/contraseña. Antes el JWT expiraba a las 4h sin renovación → había que volver a entrar cada vez. Al marcarlo, la sesión se renueva **silenciosamente** (el access token sigue siendo de 4h, pero un refresh token de 90 días lo renueva de forma transparente). Sin marcar → comportamiento idéntico al anterior (4h, sin refresh). La persistencia es **exclusivamente opt-in vía este checkbox**: registro (correo o Google) y activación corporativa entregan una sesión normal de 4h; el usuario opta por persistir en su siguiente inicio de sesión.
