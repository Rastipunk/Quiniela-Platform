  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (errorCode === "GOOGLE_ACCOUNT") {
        ...
        setResult({ type: "error", message: err?.message ?? "Error al enviar solicitud" });
      }
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div style={{ maxWidth: 520, margin: "60px auto", padding: 16 }}>
        <div className="card" style={{ padding: 24, borderRadius: 16, textAlign: "center" }}>
          ...
          <h2 style={{ marginBottom: 8 }}>Cuenta de Google</h2>
          <p style={{ color: "var(--muted)", marginBottom: 20 }}>
            Esta cuenta fue creada usando Google Sign In. No es posible restablecer la contraseña
            porque la autenticación se maneja a través de tu cuenta de Google.
          </p>
          <p
            style={{
              fontSize: 13,
          ...
          >
            Para acceder a tu cuenta, usa el botón <strong>"Iniciar con Google"</strong> en la
            página de inicio de sesión.
          </p>
          <Link href="/">
            <button style={{ width: "100%" }}>Ir al inicio de sesión</button>
          </Link>
        </div>
      </div>
    );
