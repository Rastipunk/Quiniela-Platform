...
export function AuthSlidePanel({ isOpen, onClose, onLoggedIn, initialMode }: AuthSlidePanelProps) {
  ...
  // Completar registro con Google despues de consent
  const handleGoogleConsentSubmit = async () => {
    ...
    if (!acceptAge) {
      setError("Debes confirmar que tienes al menos 13 anos");
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!GOOGLE_CLIENT_ID) {
      console.warn("NEXT_PUBLIC_GOOGLE_CLIENT_ID no configurado. Google Sign In no estara disponible.");
      return;
    }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "register") {
        ...
        if (!acceptTerms) {
          throw new Error("Debes aceptar los Terminos de Servicio");
        }
        if (!acceptPrivacy) {
          throw new Error("Debes aceptar la Politica de Privacidad");
        }
        if (!acceptAge) {
          throw new Error("Debes confirmar que tienes al menos 13 anos");
        }
  ...
  const ConsentCheckboxes = ({ inModal = false }: { inModal?: boolean }) => (
    ...
    >
      ...
      <label style={checkboxLabelStyle}>
        ...
        <span>
          Acepto los{" "}
          <Link href="/terminos" target="_blank" style={{ color: "#667eea" }}>
      ...
      <label style={checkboxLabelStyle}>
        ...
        <span>
          Acepto la{" "}
          <Link href="/privacidad" target="_blank" style={{ color: "#667eea" }}>
      ...
      <label style={checkboxLabelStyle}>
        ...
        />
        <span>
          Tengo al menos <strong>13 anos</strong>{" "}
          <span style={{ color: "#dc2626" }}>*</span>
        </span>
      </label>
        ...
        <div style={{ flex: 1, padding: 20, overflowY: "auto" }}>
          ...
          <form onSubmit={onSubmit}>
              ...
              <>
                ...
                <label style={{ display: "grid", gap: 4, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 11, color: "var(--muted)" }}>
                    3-20 caracteres, letras, numeros, _ o -
                  </span>
                </label>
            ...
            <label style={{ display: "grid", gap: 4, marginBottom: mode === "register" ? 4 : 12 }}>
              <span style={{ fontSize: 12, color: "var(--muted)", fontWeight: 500 }}>Contrasena</span>
              <input
                type="password"
                value={password}
                ...
                style={inputStyle}
              />
              <span style={{ fontSize: 11, color: "var(--muted)" }}>Minimo 8 caracteres</span>
            </label>
