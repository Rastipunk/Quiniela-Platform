...
// Standalone scoring editor — used both by the pool-creation wizard
// (via the StepScoring wrapper) and by the host "Administrar reglas"
// panel on existing DRAFT pools. The component is intentionally
// agnostic of its container: callers wrap it in whatever shell they
// need (wizard step, modal, drawer) and pass state + callbacks as
// props. Keeping it container-agnostic is why we DON'T import
// PoolWizardStepContainer here.
...
// ── Extra Time Section ────────────────────────────────────────
function ExtraTimeSection({ knockoutPhases, scoringConfig, onUpdateScoringConfig, isCustom, isMobile }: {
  knockoutPhases: PoolPickTypesConfig;
  scoringConfig: PoolPickTypesConfig;
  onUpdateScoringConfig: (config: PoolPickTypesConfig) => void;
  isCustom: boolean;
  isMobile: boolean;
}) {
  const t = useTranslations("poolWizard");
...
export interface ScoringEditorProps {
  scoringStyle: PickConfigPresetKey | null;
  scoringConfig: PoolPickTypesConfig;
  instancePhases: InstancePhase[];
  // Equivalent of dispatch({ type: "SET_SCORING", style, config }) —
  // called when the user picks a preset (or hits "Cambiar" to clear).
  onSetScoring: (style: PickConfigPresetKey | null, config: PoolPickTypesConfig) => void;
  // Equivalent of dispatch({ type: "UPDATE_SCORING_CONFIG", config }) —
  // called when the user tweaks an individual phase, criterion, or
  // multiplier.
  onUpdateScoringConfig: (config: PoolPickTypesConfig) => void;
  // Optional headings for the preset-picker view. Defaults match the
  // wizard's strings so the standalone use in "Administrar reglas"
  // can override with admin-flavored copy.
  presetPickerTitle?: string;
  presetPickerSubtitle?: string;
}
...
export function ScoringEditor({
  ...
  const applyScaling = useCallback((multipliers: Record<string, number>, base: number[]) => {
    ...
    onUpdateScoringConfig(newConfig);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scoringConfig, onUpdateScoringConfig]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scalingEnabled, scoringConfig, onUpdateScoringConfig]);
  ...
  const handleSelectPreset = useCallback((key: PickConfigPresetKey) => {
    ...
    onSetScoring(key, scaledConfig);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [instancePhases, onSetScoring]);
  ...
  if (!scoringStyle) {
    return (
      <div style={{
        ...
        {(presetPickerTitle || presetPickerSubtitle) && (
          <div>
            {presetPickerTitle && (
            ...
            {presetPickerSubtitle && (
              <p style={{ margin: "4px 0 0", fontSize: fontSize.sm, color: colors.textMuted, lineHeight: 1.5 }}>
                {presetPickerSubtitle}
              </p>
            )}
          </div>
        ...
        {(() => {
          ...
          return isCustom ? (
            <ExtraTimeSection
              ...
              scoringConfig={scoringConfig}
              onUpdateScoringConfig={onUpdateScoringConfig}
              isCustom={true}
              isMobile={isMobile}
            />
          ) : (
