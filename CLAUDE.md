...
### Results System
- **Results are scraper-first.** In AUTO mode, picks4all-scores is the ONLY result source. There is NO automatic fallback — API-Football is dead (smartSync is inert, `isAvailable() === false`). Recovery chain when the scraper can't finalize: finalization gate holds → stale-detector alert (210 min) → manual HOST_OVERRIDE.
...
- **Finalization gate (ADR-086):** the backend never counts sources — the scraper's terminal gate owns that (≥2 sources, plausible minute, hysteresis). Finalization to `API_CONFIRMED` trusts consensus `confidence`: FAST = terminal + minute ≥ `SCORES_MIN_ELAPSED_FOR_TERMINAL` (80) + confidence HIGH/VERY_HIGH; SLOW (anti-deadlock) = terminal + plausible + ≥`SCORES_SLOW_PATH_AFTER_MS` (150 min) since kickoff + confidence MEDIUM → finalize + one-time R9 admin alert. Anything else waits.
...
7. **Scraper-first results:** In AUTO mode, picks4all-scores is the ONLY source (15s polling during matches). There is NO automatic fallback (API-Football is dead; smartSync inert). Host can only override existing results.
...
14. **Scraper scoring derives from `timeline[]`, never from `fulltime`/`extratime` (always `null`).** The minute-90 / end-of-regulation score comes from `deriveNinetyMinuteScore` (the `ET` milestone of the scores-service `timeline[]`); penalties (`penaltyHome/Away`) are separate and NEVER count toward goals90. Finalization runs through the confidence-based gate in `finalizationGate.ts` (ADR-086) — never re-introduce source counting in the backend (the removed `terminalConfirmationCount` Math.max auto-finalized the false Argentina FT). Because the scraper never closes by time, these safety nets MUST stay wired: the stale detector (`staleDetector.ts`, >`SCORES_STALE_THRESHOLD_MS`/210min), the feed-silent detector (R14, `MATCH_FEED_SILENT`), the finalized-but-live alert (R11, `FINALIZED_BUT_FEED_LIVE` — alert-only, never auto-revert) and the undecidable-knockout alert (`structuralAutoPublish.ts`, `KNOCKOUT_WINNER_UNDECIDABLE`). Never reintroduce a code path that finalizes or scores a match silently when data is missing. See ADR-068 + ADR-086.
